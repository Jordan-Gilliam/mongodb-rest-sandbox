...
test('GET /articles 200 (admin)', async () => {
  const { status, body } = await request(app())
    .get(`${apiRoot}`)
...
test('GET /articles 401 (user)', async () => {
  const { status } = await request(app())
    .get(`${apiRoot}`)
...
test('GET /articles/:id 200 (admin)', async () => {
  const { status, body } = await request(app())
...
test('GET /articles/:id 401 (user)', async () => {
  const { status } = await request(app())
    .get(`${apiRoot}/${article.id}`)
    .query({ access_token: userSession })
  expect(status).toBe(401)
...
test('GET /articles/:id 404 (admin)', async () => {
  const { status } = await request(app())
